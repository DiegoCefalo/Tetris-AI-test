    ...
    def frame_step(self,input,render=True):
        ...
        if self.fallingPiece == None:
            ...
            if not self.isValidPosition():
                ...
                self.reinit()
                return image_data, reward-5, terminal # can't fit a new piece on the self.board, so game over
        ...
        if not self.isValidPosition(adjY=1):
            ...
            reward = self.score 
            # - self._number_of_holes(self.board) - self._bumpiness(self.board) - self._height(self.board)
            # - self._blocksAboveHoles(self.board)
    ...
    def get_next_states(self):
        ...
        for rotation in rotations:
            ...
            for position in range_:
                ...
                states[(position, rotation)] = torch.FloatTensor([self.simremoveCompleteLines(sim_board) , self._number_of_holes(sim_board), self._bumpiness(sim_board), self._height(sim_board), self._blocksAboveHoles(sim_board)])
